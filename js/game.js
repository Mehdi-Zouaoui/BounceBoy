bounce.game = {
    ...
    init: function(config){
        ...
        const a_textureLoader = new THREE.MTLLoader();
       ...
       // const objectiv_material = new THREE.MeshBasicMaterial( { color: 0xff0000 } );
        ...
        this.backward_direction_ray = new THREE.Vector3(0,0,-1);
        this.raycaster = new THREE.Raycaster();
        this.backward_raycaster = new THREE.Raycaster();
    ...
    a_textureLoader.setPath("3d_obj/Asteroide/"); 
    a_textureLoader.load("Asteroids.mtl",function(material){
        
        material.preload();
        
   
     loader.setMaterials(material).load("3d_obj/Asteroide/Asteroids.OBJ",function (ast) {
    ...
    update : function(){
        ...
        game.score.innerHTML= Number(game.score.innerHTML) + 1;
        ...
        if(game.objectivs.length > 0){
            ...
            game.backward_raycaster.set(game.falcon.position, game.backward_direction_ray);
            const intersects = game.raycaster.intersectObjects(game.objectivs, false);
            const backward_intersects = game.raycaster.intersectObjects(game.objectivs, false);
             
            if (intersects.length> 0 ){
                
            for(let j = 0 ; j < intersects.length ; j++ ){
                console.log(intersects[j].distance);
                if(intersects[j].distance < 5){
